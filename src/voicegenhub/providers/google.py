    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if not creds_path and not creds_json:
                logger.warning(
                    "No Google Cloud credentials found. Provider will be unavailable."
                )
                return
    ...
    async def _test_connection(self) -> None:
        ...
        try:
            ...
            if not voices.voices:
                raise TTSError(
                    "No voices returned from Google TTS", provider=self.provider_id
                )
        except Exception as e:
            if "authentication" in str(e).lower():
                raise AuthenticationError(
                    f"Google TTS authentication failed: {e}", provider=self.provider_id
                )
            raise TTSError(
                f"Google TTS connection test failed: {e}", provider=self.provider_id
            )
    ...
    async def synthesize(self, request: TTSRequest) -> TTSResponse:
        ...
        try:
            ...
            voice_config = texttospeech.VoiceSelectionParams(
                name=request.voice_id, language_code=request.language or "en-US"
            )
            ...
            logger.info(
                f"Successfully generated {duration:.2f}s of audio using Google TTS"
            )
            return tts_response
