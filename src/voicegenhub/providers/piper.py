...
class PiperTTSProvider(TTSProvider):
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if not self.model_path:
                ...
                possible_paths = [
                    os.path.expanduser(
                        "~/.local/share/piper/models/en_US-lessac-medium.onnx"
                    ),
                    os.path.expanduser(
                        "~/.cache/piper/models/en_US-lessac-medium.onnx"
                    ),
            ...
            if not self.model_path:
                logger.warning(
                    "No Piper model path configured. Models will be downloaded on first use."
                )
            else:
            ...
            logger.warning(
                "Piper TTS provider will be disabled. Install with: pip install piper-tts"
            )
    ...
    async def get_voices(self, language: Optional[str] = None) -> List[Voice]:
        ...
        if self._initialization_failed:
            raise TTSError(
                "Piper TTS provider not properly initialized. "
                "Please ensure piper-tts and onnxruntime are installed."
            )
        ...
        if self._voices_cache:
            ...
            try:
                ...
                download_dir = self.config.get("download_dir") or os.path.expanduser(
                    "~/.local/share/piper"
                )
                ...
                for voice_id, voice_info in voices_data.items():
                    ...
                    try:
                        ...
                        gender = (
                            VoiceGender.FEMALE
                            if any(
                                x in speaker_name.lower()
                                for x in ["she", "female", "woman"]
                            )
                        ...
                        locale = (
                            f"{locale_parts[0]}-{locale_parts[1].upper()}"
                        ...
                        parsed_voice = Voice(
                            ...
                            description=voice_info.get(
                                "description", "Piper neural voice"
                            ),
                        )
                ...
                logger.error(f"Could not fetch dynamic Piper voices: {e}")
                raise TTSError(
                    f"Failed to fetch Piper voices: {e}. Please ensure piper-tts is properly installed.",
    ...
    async def synthesize(self, request: TTSRequest) -> TTSResponse:
        ...
        try:
            ...
            if request.speed != 1.0:
                ...
                logger.warning(
                    f"Piper TTS doesn't support speed control. Using default speed (ignoring {request.speed})"
                )
